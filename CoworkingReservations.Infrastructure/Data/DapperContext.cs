...
namespace CoworkingReservations.Infrastructure.Data
{
    public class DapperContext : IDapperContext
    {
        ...
        private (IDbConnection conn, IDbTransaction? tx, bool ownsConnection) GetConnAndTx()
        {
            ...
            {
                return (ambient.Conn, ambient.Tx, false); // Usa la conexión/transacción ambiental
            }
            var conn = _connFactory.CreateConnection(); // Crea una nueva
        ...
        private async Task CloseAndDisposeIfNeededAsync(IDbConnection conn, bool ownsConnection)
        {
            if (ownsConnection)
            {
                if (conn is DbConnection dbConn && dbConn.State != ConnectionState.Closed)
        ...
        public async Task<IEnumerable<T>> QueryAsync<T>(string sql, object? param = null, CommandType commandType = CommandType.Text)
        {
            ...
            {
                await CloseAndDisposeIfNeededAsync(conn, owns);
            }
        }
